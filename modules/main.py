...
async def start(client: Client, msg: Message):
    ...
    await start_message.edit_text(
        Data.START.format(msg.from_user.mention) +
        "Checking status Ok... Command Nhi Bataunga **Bot Made BY 𝔸𝕝𝕖𝕩 𝕁𝔸𝔸𝕋™👨🏻‍💻**🔍\n\n"
        "Progress:[🟩🟩🟩🟩🟩🟩🟩🟩🟩] 100%\n\n"
    )
    ...
    try:
        for i in range(arg-1, len(links)):
            ...
            if "edge.api.brightcove.com" in url:
                ...
                cmd = f'yt-dlp --cookies youtube_cookies.txt -o "{name}.mp4" "{url}"'
...
@bot.on_message(filters.command(["nnl"]) )
async def txt_handler(bot: Client, m: Message):
    ...
    try:
        for i in range(arg-1, len(links)):
            ...
            try:  
                ...
                cc = f'**🎞️ VID_ID: {str(count).zfill(3)}.\n\nTitle: {name1} 𝔸𝕝𝕖𝕩 𝕁𝔸𝔸𝕋 {res}.mkv\n\n📚 Batch Name: {b_name}\n\n📥 Extracted By : {CR}\n\n**━━━━━✦📖🇮🇳📖✦━━━━━**'
                cc1 = f'**📁 PDF_ID: {str(count).zfill(3)}.\n\nTitle: {name1} 𝔸𝕝𝕖𝕩 𝕁𝔸𝔸𝕋.pdf\n\n📚 Batch Name: {b_name}\n\n📥 Extracted By : {CR}\n\n**━━━━━✦📖🇮🇳📖✦━━━━━**'
...
@bot.on_message(filters.command(["alex"]))
async def txt_handler(bot: Client, m: Message):
    editable = await m.reply_text(f"**🔹Send me the TXT file and wait.**")
    input: Message = await bot.listen(editable.chat.id)
    x = await input.download()
